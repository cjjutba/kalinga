...
export function ClinicPage({ data }: { data: PublicClinic }) {
  ...
  return (
    <>
      <main className="mx-auto w-full max-w-5xl px-4 pb-6 md:px-6 lg:py-10">
      <div className="lg:grid lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1fr)] lg:items-start lg:gap-12">
        {/* Who and where, and the one button. It stays in view on a laptop
            while the services are read beside it. */}
        <div className="lg:sticky lg:top-10">
          <Photo className="-mx-4 h-56 rounded-b-sheet md:mx-0 md:mt-6 md:h-72 md:rounded-sheet lg:mt-0 lg:h-64" caption="Photograph, the clinic's own" />
          <h1 className="mt-6 text-title font-medium text-balance">{org.name}</h1>
          ...
          <div className="mt-6">
            {bookable ? (
              <>
                <Pill asChild block className="lg:w-auto lg:px-8">
                  <Link href={`/${org.slug}/book`}>Book an appointment</Link>
                </Pill>
                <p className="mt-2 text-center text-label text-text-2 lg:text-left">Under a minute. No account needed.</p>
              </>
            ) : (
        ...
        <div className="mt-10 flex flex-col gap-8 lg:mt-0">
          ...
          <section className="rounded-card bg-tint p-5" aria-labelledby="recall">
